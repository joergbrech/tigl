...
{
    return _cache->_chordSurface->Value(xsi, eta);
}

void CTiglWingChordface::GetEtaXsi(gp_Pnt point, double &eta, double &xsi) const
{
    GeomAPI_ProjectPointOnSurf projector(point, _cache->_chordSurface, 0., 1., 0., 1.);
...
{
    TopoDS_Face face = BRepBuilderAPI_MakeFace(_cache->_chordSurface, Precision::Confusion());
...
void CTiglWingChordface::BuildChordSurface(ChordSurfaceCache& cache) const
{
    ...
    cache._chordSurface = new Geom_BSplineSurface(poles, uKnots, vKnots, uMults, vMults, 1, 1);

    // set the break points
    for (int iElement = 1; iElement <= cache._chordSurface->NbVKnots(); ++iElement) {
        cache._elementEtas.push_back(cache._chordSurface->VKnot(iElement));
    }
}

const std::vector<double>& CTiglWingChordface::GetElementEtas() const
{
    return _cache->_elementEtas;
}

const Handle(Geom_BSplineSurface) CTiglWingChordface::GetSurface() const
{
    return _cache->_chordSurface;
}
